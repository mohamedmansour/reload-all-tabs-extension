...
export const updateContextMenu = async () => {
  ...
  try {
    ...
    const menuItems = [
      ...
      { id: 'reloadSkipMatched', enabled: setting.reloadSkipMatched, title: `Reload all tabs except matched urls${attributions}` }
    ];
...
export const onMenuClicked = async (info, tab) => {
  ...
  const menuActions = {
    ...
    reloadSkipMatched: () => reloadWindow({ id: windowId }, { reloadSkipMatched: true }),
